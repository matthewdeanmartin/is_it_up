...
def get_packages():
    if not GEM_FURY:
        raise Exception("Can't get_packages, no gem_fury account.")
    packages = []
...
def get_versions():
    if not GEM_FURY:
        raise Exception("Can't get_versions, no gem_fury account.")
    versions = []
    command = "fury versions {0} --as={1}".format(PROJECT_NAME, GEM_FURY)
